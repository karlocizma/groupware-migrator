...
function statusClass(status, running) {
  if (running && status !== "completed" && status !== "failed") {
    return "running";
  }
  return status;
}
...
function applyOAuthConnectionFields(connection, prefix) {
  ...
  if (accessToken) {
    connection.oauth_access_token = accessToken;
  }
  if (refreshToken) {
    connection.oauth_refresh_token = refreshToken;
  }
  if (clientId) {
  ...
  if (tokenUrl) {
    connection.oauth_token_url = tokenUrl;
  }
  if (scope) {
...
function renderPreflightResult(result) {
  ...
  const cursorCollections = Number(
    result.incremental?.resolved_cursor_collections ??
      result.incremental?.resolved_cursor_mailboxes ??
      0,
  );
  ...
  output.innerHTML = `
    ...
    <div class="preflight-grid">
      ...
      <div class="preflight-item ${incrementalOk ? "ok" : "failed"}">
        <strong>Sync</strong>
        <span>${incrementalMode.toUpperCase()}</span>
        <div>${
          incrementalMode === "incremental"
            ? `${cursorCollections} cursor collection(s) from ${resolutionSource}.`
            : "Full sync selected."
        }</div>
    ...
    ${
      warnings.length
        ? `<div class="preflight-warnings"><strong>Warnings:</strong><ul>${warnings
...
function renderBatchPreview(preview) {
  ...
  panel.innerHTML = `
    ${
      hasPreflight
        ? `<div class="batch-preview-summary">Preflight checked ${checkedRows} row(s): ${okRows} ok / ${failedRows} failed.</div>`
        : ""
    }
    ${preview.items
      .map((item) => {
        const hasRowPreflight =
          Object.prototype.hasOwnProperty.call(item, "preflight") ||
          Object.prototype.hasOwnProperty.call(item, "preflight_skipped");
        ...
        if (hasRowPreflight) {
          if (preflightSkipped) {
            ...
            const errorParts = [];
            if (preflight.source && !preflight.source.ok && preflight.source.error) {
              errorParts.push(`source: ${preflight.source.error}`);
            }
            if (preflight.destination && !preflight.destination.ok && preflight.destination.error) {
              errorParts.push(`destination: ${preflight.destination.error}`);
            }
            if (preflight.plan && !preflight.plan.ok && preflight.plan.error) {
              errorParts.push(`plan: ${preflight.plan.error}`);
            }
            preflightError = errorParts.length
              ? `<div class="preview-error">${errorParts.join(" | ")}</div>`
              : "";
          } else {
...
function renderJobDetails(job) {
  ...
  const estimated = Number(
    job.plan_summary?.total_estimated_items ?? job.plan_summary?.total_estimated_messages ?? 0,
  );
  ...
  panel.innerHTML = `
    ...
    ${
      job.last_error
        ? `<div class="error-box"><strong>Last error:</strong><br>${job.last_error}</div>`
        : ""
    }
  `;
          ...
          ? items
              .map(
                ...
                <div class="batch-preview-row ${item.status === "failed" ? "invalid" : ""}">
                  ...
                  ${
                    item.last_error
                      ? `<div class="preview-error">${item.last_error}</div>`
                      : ""
                  }
                </div>
              `,
              ...
              .join("")
          : `<div class="batch-preview empty">No rows available.</div>`
      }
    </div>
  `;
}
...
async function buildPlan() {
  try {
    ...
    const plan = await requestJSON(`${API_BASE}/jobs/plan`, {
      method: "POST",
...
async function runJobPreflight() {
  try {
    ...
    const request = buildRequestPayload();
    const result = await requestJSON(`${API_BASE}/jobs/preflight`, {
      method: "POST",
      body: JSON.stringify(request),
    });
    ...
    setFeedback(
      result.overall_ok
        ...
        : "Preflight checks completed with failures.",
...
async function startBackgroundJob() {
  try {
    ...
    const request = buildRequestPayload();
    const response = await requestJSON(`${API_BASE}/jobs/start`, {
      method: "POST",
      body: JSON.stringify(request),
    });
...
async function previewBatchCsv() {
  try {
    ...
    const preview = await requestJSON(`${API_BASE}/batches/preview`, {
      method: "POST",
...
async function startBatchMigration() {
  try {
    ...
    const payload = buildBatchPayload();
    const batch = await requestJSON(`${API_BASE}/batches/start`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
...
async function runBatchPreflight() {
  try {
    ...
    const result = await requestJSON(`${API_BASE}/batches/preflight`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
...
function wireInteractions() {
  ...
  wireDisclosureToggle(
    ...
    "batch-advanced-panel",
    "Show batch advanced settings",
    "Hide batch advanced settings",
  );
  $("plan-button").addEventListener("click", buildPlan);
  $("preflight-button").addEventListener("click", runJobPreflight);
  $("start-button").addEventListener("click", startBackgroundJob);
  ...
  $("source-provider").addEventListener("change", () => {
    applySourceProviderDefaults();
  });
  $("source-auth-mode").addEventListener("change", () => {
    setOAuthFieldVisibility("source");
  });
  $("destination-provider").addEventListener("change", () => {
    applyDestinationProviderDefaults();
  });
  $("destination-auth-mode").addEventListener("change", () => {
    setOAuthFieldVisibility("destination");
  });
  $("sync-mode").addEventListener("change", () => {
    setSyncModeVisibility();
  });
  $("batch-preview-button").addEventListener("click", previewBatchCsv);
  $("batch-preflight-button").addEventListener("click", runBatchPreflight);
  $("batch-start-button").addEventListener("click", startBatchMigration);
  ...
  $("batch-csv-file").addEventListener("change", (event) => {
    ...
    loadCsvFileToTextarea(fileInput).catch((error) => {
